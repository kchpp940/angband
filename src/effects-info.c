...
/**
 * Returns the range (maximum distance) of an effect, or 0 if it has no
 * meaningful range (self-targeted, melee, etc.).
 */
int effect_range(const struct effect *effect)
{
	if (effect->index == EF_RANDOM || effect->index == EF_SELECT) {
		...
		for (e = effect->next; e != NULL && num_subeffects > 0;
				e = e->next, --num_subeffects) {
			int r = effect_range(e);
			if (r > max_r) max_r = r;
		}
		return max_r;
	}
...
/**
 * Returns the radius of an area effect, or 0 if it does not have a radius.
 * For level-scaling radii, returns the effective radius at the player's
 * current level.
 */
