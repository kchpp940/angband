...
void init_arrays(void)
{
	...
	for (i = 0; i < N_ELEMENTS(pl); i++) {
		...
		parse_err = run_parser(pl[i].parser);
		if (parse_err) {
			if (global_init_result) {
				const char *err_desc = (parse_err > 0 && parse_err < PARSE_ERROR_MAX)
					? parser_error_str[parse_err] : "unspecified error";
				init_record_error(global_init_result, INIT_STAGE_ARRAYS, parse_err,
					"Parser '%s' failed: %s", pl[i].name, err_desc);
			}
...
void cleanup_angband(void)
{
	...
	/* Free the global init result if it exists */
	if (global_init_result) {
		init_destroy_result(global_init_result);
		global_init_result = NULL;
	}
}

/**
 * ------------------------------------------------------------------------
 * Unified initialization system with staged loading and error tracking
 * ------------------------------------------------------------------------
 */

struct init_result *global_init_result = NULL;

static void init_stage_setup_defaults(struct init_result *result)
{
	int i;

	for (i = 0; i < INIT_STAGE_MAX; i++) {
		result->stages[i].stage = (init_stage_t)i;
		result->stages[i].status = INIT_STATUS_PENDING;
		result->stages[i].error_code = 0;
		result->stages[i].error_message[0] = '\0';
		result->stages[i].dependency_count = 0;
	}

	result->stages[INIT_STAGE_PATHS].name = "paths";
	result->stages[INIT_STAGE_PATHS].description = "File path initialization";
	result->stages[INIT_STAGE_PATHS].dependency_count = 0;

	result->stages[INIT_STAGE_DIRS].name = "dirs";
	result->stages[INIT_STAGE_DIRS].description = "Required directory creation";
	result->stages[INIT_STAGE_DIRS].dependency_count = 1;
	result->stages[INIT_STAGE_DIRS].dependencies[0] = INIT_STAGE_PATHS;

	result->stages[INIT_STAGE_MESSAGES].name = "messages";
	result->stages[INIT_STAGE_MESSAGES].description = "Message system initialization";
	result->stages[INIT_STAGE_MESSAGES].dependency_count = 1;
	result->stages[INIT_STAGE_MESSAGES].dependencies[0] = INIT_STAGE_PATHS;

	result->stages[INIT_STAGE_CONSTANTS].name = "constants";
	result->stages[INIT_STAGE_CONSTANTS].description = "Game constants (constants.txt)";
	result->stages[INIT_STAGE_CONSTANTS].dependency_count = 2;
	result->stages[INIT_STAGE_CONSTANTS].dependencies[0] = INIT_STAGE_PATHS;
	result->stages[INIT_STAGE_CONSTANTS].dependencies[1] = INIT_STAGE_MESSAGES;

	result->stages[INIT_STAGE_VISUALS].name = "visuals";
	result->stages[INIT_STAGE_VISUALS].description = "UI visuals and rendering data";
	result->stages[INIT_STAGE_VISUALS].dependency_count = 2;
	result->stages[INIT_STAGE_VISUALS].dependencies[0] = INIT_STAGE_PATHS;
	result->stages[INIT_STAGE_VISUALS].dependencies[1] = INIT_STAGE_CONSTANTS;

	result->stages[INIT_STAGE_ARRAYS].name = "arrays";
	result->stages[INIT_STAGE_ARRAYS].description = "Game data arrays (projections, classes, monsters, objects, etc.)";
	result->stages[INIT_STAGE_ARRAYS].dependency_count = 3;
	result->stages[INIT_STAGE_ARRAYS].dependencies[0] = INIT_STAGE_CONSTANTS;
	result->stages[INIT_STAGE_ARRAYS].dependencies[1] = INIT_STAGE_MESSAGES;
	result->stages[INIT_STAGE_ARRAYS].dependencies[2] = INIT_STAGE_VISUALS;

	result->stages[INIT_STAGE_PLAYER].name = "player";
	result->stages[INIT_STAGE_PLAYER].description = "Player system initialization";
	result->stages[INIT_STAGE_PLAYER].dependency_count = 1;
	result->stages[INIT_STAGE_PLAYER].dependencies[0] = INIT_STAGE_ARRAYS;

	result->stages[INIT_STAGE_GENERATE].name = "generate";
	result->stages[INIT_STAGE_GENERATE].description = "Dungeon generation subsystem";
	result->stages[INIT_STAGE_GENERATE].dependency_count = 1;
	result->stages[INIT_STAGE_GENERATE].dependencies[0] = INIT_STAGE_ARRAYS;

	result->stages[INIT_STAGE_RUNES].name = "runes";
	result->stages[INIT_STAGE_RUNES].description = "Rune system initialization";
	result->stages[INIT_STAGE_RUNES].dependency_count = 1;
	result->stages[INIT_STAGE_RUNES].dependencies[0] = INIT_STAGE_ARRAYS;

	result->stages[INIT_STAGE_OBJ_MAKE].name = "obj_make";
	result->stages[INIT_STAGE_OBJ_MAKE].description = "Object creation/allocation subsystem";
	result->stages[INIT_STAGE_OBJ_MAKE].dependency_count = 1;
	result->stages[INIT_STAGE_OBJ_MAKE].dependencies[0] = INIT_STAGE_ARRAYS;

	result->stages[INIT_STAGE_IGNORE].name = "ignore";
	result->stages[INIT_STAGE_IGNORE].description = "Object ignore system";
	result->stages[INIT_STAGE_IGNORE].dependency_count = 1;
	result->stages[INIT_STAGE_IGNORE].dependencies[0] = INIT_STAGE_ARRAYS;

	result->stages[INIT_STAGE_MON_MAKE].name = "mon_make";
	result->stages[INIT_STAGE_MON_MAKE].description = "Monster creation subsystem";
	result->stages[INIT_STAGE_MON_MAKE].dependency_count = 1;
	result->stages[INIT_STAGE_MON_MAKE].dependencies[0] = INIT_STAGE_ARRAYS;

	result->stages[INIT_STAGE_STORE].name = "store";
	result->stages[INIT_STAGE_STORE].description = "Store system initialization";
	result->stages[INIT_STAGE_STORE].dependency_count = 1;
	result->stages[INIT_STAGE_STORE].dependencies[0] = INIT_STAGE_ARRAYS;

	result->stages[INIT_STAGE_OPTIONS].name = "options";
	result->stages[INIT_STAGE_OPTIONS].description = "Game options system";
	result->stages[INIT_STAGE_OPTIONS].dependency_count = 1;
	result->stages[INIT_STAGE_OPTIONS].dependencies[0] = INIT_STAGE_ARRAYS;

	result->stages[INIT_STAGE_UI_PLAYER].name = "ui_player";
	result->stages[INIT_STAGE_UI_PLAYER].description = "UI player display subsystem";
	result->stages[INIT_STAGE_UI_PLAYER].dependency_count = 2;
	result->stages[INIT_STAGE_UI_PLAYER].dependencies[0] = INIT_STAGE_PLAYER;
	result->stages[INIT_STAGE_UI_PLAYER].dependencies[1] = INIT_STAGE_ARRAYS;

	result->stages[INIT_STAGE_UI_EQUIP_CMP].name = "ui_equip_cmp";
	result->stages[INIT_STAGE_UI_EQUIP_CMP].description = "UI equipment comparison subsystem";
	result->stages[INIT_STAGE_UI_EQUIP_CMP].dependency_count = 2;
	result->stages[INIT_STAGE_UI_EQUIP_CMP].dependencies[0] = INIT_STAGE_PLAYER;
	result->stages[INIT_STAGE_UI_EQUIP_CMP].dependencies[1] = INIT_STAGE_ARRAYS;

	result->stages[INIT_STAGE_LISTS].name = "lists";
	result->stages[INIT_STAGE_LISTS].description = "Monster and object list initialization";
	result->stages[INIT_STAGE_LISTS].dependency_count = 1;
	result->stages[INIT_STAGE_LISTS].dependencies[0] = INIT_STAGE_ARRAYS;

	result->stages[INIT_STAGE_RNG].name = "rng";
	result->stages[INIT_STAGE_RNG].description = "Random number generator initialization";
	result->stages[INIT_STAGE_RNG].dependency_count = 0;

	result->stages[INIT_STAGE_COMPLETE].name = "complete";
	result->stages[INIT_STAGE_COMPLETE].description = "All initialization stages complete";
	result->stages[INIT_STAGE_COMPLETE].dependency_count = 0;
}

struct init_result *init_create_result(void)
{
	struct init_result *result = mem_zalloc(sizeof(*result));
	result->success = false;
	result->failed_stage = INIT_STAGE_NOT_STARTED;
	result->error_summary[0] = '\0';
	init_stage_setup_defaults(result);
	return result;
}

void init_destroy_result(struct init_result *result)
{
	if (result) {
		mem_free(result);
	}
...
void init_reset_result(struct init_result *result)
{
	if (!result) return;
	result->success = false;
	result->failed_stage = INIT_STAGE_NOT_STARTED;
...
const char *init_stage_name(init_stage_t stage)
{
	switch (stage) {
		case INIT_STAGE_NOT_STARTED: return "not_started";
		case INIT_STAGE_PATHS: return "paths";
		case INIT_STAGE_DIRS: return "dirs";
		case INIT_STAGE_MESSAGES: return "messages";
		case INIT_STAGE_CONSTANTS: return "constants";
		case INIT_STAGE_VISUALS: return "visuals";
		case INIT_STAGE_ARRAYS: return "arrays";
		case INIT_STAGE_PLAYER: return "player";
		case INIT_STAGE_GENERATE: return "generate";
		case INIT_STAGE_RUNES: return "runes";
		case INIT_STAGE_OBJ_MAKE: return "obj_make";
		case INIT_STAGE_IGNORE: return "ignore";
		case INIT_STAGE_MON_MAKE: return "mon_make";
		case INIT_STAGE_STORE: return "store";
		case INIT_STAGE_OPTIONS: return "options";
		case INIT_STAGE_UI_PLAYER: return "ui_player";
		case INIT_STAGE_UI_EQUIP_CMP: return "ui_equip_cmp";
		case INIT_STAGE_LISTS: return "lists";
		case INIT_STAGE_RNG: return "rng";
		case INIT_STAGE_COMPLETE: return "complete";
		default: return "unknown";
	}
}

const char *init_status_name(init_status_t status)
{
	switch (status) {
		case INIT_STATUS_PENDING: return "pending";
		case INIT_STATUS_IN_PROGRESS: return "in_progress";
		case INIT_STATUS_COMPLETE: return "complete";
		case INIT_STATUS_FAILED: return "failed";
		default: return "unknown";
	}
}

bool init_stage_has_completed(struct init_result *result, init_stage_t stage)
{
	if (!result || stage <= INIT_STAGE_NOT_STARTED || stage >= INIT_STAGE_MAX)
		return false;
	return result->stages[stage].status == INIT_STATUS_COMPLETE;
}

bool init_check_dependencies(struct init_result *result, init_stage_t stage)
{
	int i;
	struct init_stage_info *info;
	...
	for (i = 0; i < info->dependency_count; i++) {
		init_stage_t dep = info->dependencies[i];
		if (result->stages[dep].status != INIT_STATUS_COMPLETE) {
			return false;
		}
	}
	return true;
...
void init_record_error(struct init_result *result, init_stage_t stage, errr code, const char *fmt, ...)
{
	va_list args;
	struct init_stage_info *info;
	...
	info = &result->stages[stage];
	info->status = INIT_STATUS_FAILED;
	...
	snprintf(result->error_summary, sizeof(result->error_summary),
		"Initialization failed at stage '%s' (%s): %s",
		init_stage_name(stage), info->description, info->error_message);
}

void init_set_stage_status(struct init_result *result, init_stage_t stage, init_status_t status)
{
	if (!result || stage <= INIT_STAGE_NOT_STARTED || stage >= INIT_STAGE_MAX)
		return;
	result->stages[stage].status = status;
}

static bool init_execute_module(struct init_result *result, init_stage_t stage,
	struct init_module *module)
{
...
bool init_run_stage(struct init_result *result, init_stage_t stage)
{
	struct init_stage_info *info;
	...
	if (info->status == INIT_STATUS_COMPLETE)
		return true;

	if (info->status == INIT_STATUS_FAILED)
		return false;

	if (!init_check_dependencies(result, stage)) {
		init_record_error(result, stage, PARSE_ERROR_GENERIC,
			"Missing required dependencies for stage '%s'", info->name);
		return false;
	}

	info->status = INIT_STATUS_IN_PROGRESS;

	switch (stage) {
		case INIT_STAGE_PATHS:
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_DIRS:
			create_needed_dirs();
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_MESSAGES:
			if (!init_execute_module(result, stage, &messages_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_CONSTANTS:
			init_game_constants();
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_VISUALS:
			if (!init_execute_module(result, stage, &ui_visuals_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_ARRAYS:
			init_arrays();
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_PLAYER:
			if (!init_execute_module(result, stage, &player_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_GENERATE:
			if (!init_execute_module(result, stage, &generate_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_RUNES:
			if (!init_execute_module(result, stage, &rune_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_OBJ_MAKE:
			if (!init_execute_module(result, stage, &obj_make_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_IGNORE:
			if (!init_execute_module(result, stage, &ignore_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_MON_MAKE:
			if (!init_execute_module(result, stage, &mon_make_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_STORE:
			if (!init_execute_module(result, stage, &store_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_OPTIONS:
			if (!init_execute_module(result, stage, &options_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_UI_PLAYER:
			if (!init_execute_module(result, stage, &ui_player_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_UI_EQUIP_CMP:
			if (!init_execute_module(result, stage, &ui_equip_cmp_module)) {
				return false;
			}
			info->status = INIT_STATUS_COMPLETE;
			break;

		case INIT_STAGE_LISTS:
			monster_list_init();
			object_list_init();
			info->status = INIT_STATUS_COMPLETE;
			break;
...
bool init_run_all_stages(struct init_result *result)
{
	init_stage_t stage;
	...
	for (stage = INIT_STAGE_PATHS; stage < INIT_STAGE_COMPLETE; stage = (init_stage_t)((int)stage + 1)) {
		if (!init_run_stage(result, stage)) {
			result->success = false;
			return false;
		}
	}

	init_set_stage_status(result, INIT_STAGE_COMPLETE, INIT_STATUS_COMPLETE);
	result->success = true;
	return true;
}

void init_print_report(struct init_result *result)
{
	int i;

	if (!result) return;

	plog("=== Initialization Status Report ===");
	for (i = 0; i < INIT_STAGE_MAX; i++) {
		struct init_stage_info *info = &result->stages[i];
		if (info->status == INIT_STATUS_PENDING && i > 0)
			continue;

		plog_fmt("  [%s] %-16s %s",
			init_status_name(info->status),
			init_stage_name(info->stage),
			info->description ? info->description : "");

		if (info->status == INIT_STATUS_FAILED && info->error_message[0]) {
			plog_fmt("    ERROR: %s (code=%d)", info->error_message, (int)info->error_code);
		}
	}
	if (!result->success && result->error_summary[0]) {
		plog_fmt("SUMMARY: %s", result->error_summary);
	}
	plog("===================================");
}

bool init_setup_paths(struct init_result *result, const char *config, const char *lib, const char *data)
{
	if (!result) return false;

	init_set_stage_status(result, INIT_STAGE_PATHS, INIT_STATUS_IN_PROGRESS);
	init_file_paths(config, lib, data);
	init_set_stage_status(result, INIT_STAGE_PATHS, INIT_STATUS_COMPLETE);
	return true;
}

bool init_angband_with_result(struct init_result *result)
{
	if (!result) return false;
	...
	return init_run_all_stages(result);
}

bool init_test_data_only(struct init_result *result)
{
	if (!result) return false;
	...
	if (!init_run_stage(result, INIT_STAGE_MESSAGES)) return false;
	if (!init_run_stage(result, INIT_STAGE_CONSTANTS)) return false;
	if (!init_run_stage(result, INIT_STAGE_VISUALS)) return false;
	if (!init_run_stage(result, INIT_STAGE_ARRAYS)) return false;
