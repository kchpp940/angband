...
/**
 * Read a slot string safely into a buffer.  Returns true on success.
 * On failure the slot is marked as unused.
 */
static bool rd_slot_string(char *buf, size_t bufsz)
{
	size_t i = 0;
	uint8_t tmp8u;
...
/**
 * Read v1 equipment sets (original format without pval/ac/weight/origin/inscription).
 * Used for backward compatibility with older saves.
 * Missing fields are zeroed / NULLed.
 */
int rd_equip_sets_v1(void)
{
	...
	for (i = 0; i < max_sets; i++) {
		...
		if (valid) {
			...
			if (!rd_slot_string(buf, sizeof(buf))) {
				return 0;
			}
			if (buf[0] != '\0') {
...
/**
 * Read v2 (current) equipment sets.
 * Contains full stable identification fields, and is hardened against
 * corrupted, truncated, or oversized save data.
 */
	...
	for (i = 0; i < max_sets; i++) {
		...
		if (valid) {
			...
			for (j = 0; j < num_slots; j++) {
				...
				rd_byte(&slot->ds);
				rd_byte(&slot->origin);
				rd_byte(&slot->origin_depth);

				if (!rd_slot_string(buf, sizeof(buf))) {
					return 0;
				}
				if (buf[0] != '\0') {
					slot->inscription = string_make(buf);
				} else {
					slot->inscription = NULL;
				}
