...
/**
 * GCU (curses) frontend handler for danger events. Directly calls the
 * Term-level bell for critical HP danger, plus the terminal's visual-bell
 * escape sequence (flash()) for all critical danger. flash() is standard
 * Curses and falls back to an audible bell automatically.
 */
...
/**
 * GCU (curses) frontend handler for end-of-frame flush. Directly calls
 * Term_fresh(). Curses uses wrefresh() per subwindow already; this is a
 * hook point for future terminal-specific optimizations (e.g. doupdate()).
 */
...
/**
 * GCU (curses) frontend handler for highlighted messages. Directly composes
 * event data and calls display_message() + message_flush().
 */
static void gcu_handle_message_highlight(game_event_type type,
...
/**
 * GCU (curses) frontend handler for status bar repaint requests. Merges flags
 * and calls update_statusline_aux() + update_sidebar() directly.
 */
static void gcu_handle_statusbar(game_event_type type, game_event_data *ev_data,
								 void *user)
{
	if (!ev_data) return;
...
/**
 * GCU (curses) frontend handler for full or partial map redraw requests.
 */
static void gcu_handle_map_redraw(game_event_type type, game_event_data *ev_data,
								  void *user)
{
	if (!ev_data) return;
...
/**
 * GCU (curses) frontend handler for subwindow repaints. Iterates all subwindows.
 */
static void gcu_handle_subwindow(game_event_type type, game_event_data *ev_data,
								 void *user)
{
	int i;
	term *old = Term;
							...
							void *user)
{
	...
	event_add_handler(EVENT_MESSAGE_HIGHLIGHT,
					  gcu_handle_message_highlight, NULL);
	event_add_handler(EVENT_STATUSBAR, gcu_handle_statusbar, NULL);
	event_add_handler(EVENT_MAP_REDRAW, gcu_handle_map_redraw, NULL);
	event_add_handler(EVENT_SUBWINDOW, gcu_handle_subwindow, NULL);
							...
							void *user)
{
	...
	event_remove_handler(EVENT_MESSAGE_HIGHLIGHT,
						 gcu_handle_message_highlight, NULL);
	event_remove_handler(EVENT_STATUSBAR, gcu_handle_statusbar, NULL);
	event_remove_handler(EVENT_MAP_REDRAW, gcu_handle_map_redraw, NULL);
	event_remove_handler(EVENT_SUBWINDOW, gcu_handle_subwindow, NULL);
