 ...
 *
 * The "lib/user/pref-ibm.prf" file contains keymap definitions and possible
...
static uint8_t ext_color_map[32] = {
	...
	14, /* Teal -> Light Blue */
	7,  /* Mud -> Umber */
	15, /* Light Yellow -> Light Umber */
	10, /* Magenta -> Violet */
	14, /* Light Teal -> Light Blue */
	10, /* Light Violet -> Violet */
	12, /* Light Pink -> Light Red */
	7,  /* Mustard -> Yellow */
	6,  /* Blue Slate -> Blue */
	14, /* Deep Light Blue -> Light Blue */
	8   /* Shade -> Light Dark */
};
...
static int get_ext_color(int a)
{
	int retval;
	if (a > -1 && a < 29) {
		retval = ext_color_map[a];
	} else {
		retval = a & 0x0F;
	}
	return retval;
 ...
 *
 * To simplify this file, we use Angband's "keymap" ability, in
 ...
 * delimit the encoding for recognition by the keymap code.
 *
 ...
 *
 * See "pref-ibm.prf" for the "standard" keymaps for various keys.
 *
 ...
 * to be used in keymaps without hurting their use in normal situations.
 */
static errr Term_xtra_ibm_event(int v)
{
	...
	case 0x56:
		...
		break;
	case 0x35: kp = true; ch = '/'; break;
	case 0x37: kp = true; ch = '*'; break;
	case 0x47: kp = true; ch = '7'; break;
	case 0x48: kp = true; ch = '8'; break;
	case 0x49: kp = true; ch = '9'; break;
	case 0x4A: kp = true; ch = '-'; break;
	case 0x4B: kp = true; ch = '4'; break;
	case 0x4C: kp = true; ch = '5'; break;
	case 0x4D: kp = true; ch = '6'; break;
	case 0x4E: kp = true; ch = '+'; break;
	case 0x4F: kp = true; ch = '1'; break;
	case 0x50: kp = true; ch = '2'; break;
	case 0x51: kp = true; ch = '3'; break;
	case 0x52: kp = true; ch = '0'; break;
	case 0x53: kp = true; ch = '.'; break;
	case 0x95:
		if (mc) {
			kp = true;
			ch = '/';
...
static errr Term_text_ibm(int x, int y, int n, int a, const wchar_t *cp)
{
	...
	else
	{
		...
		attr = ibm_color_simple[get_ext_color(a)];
	}
 ...
 * The given parameters are "valid".
 */
static errr Term_pict_ibm(int x, int y, int n, const int *ap,
			  const wchar_t *cp, const int *tap, const wchar_t *tcp)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		{
			/* Extract a color value */
			attr = ibm_color_simple[get_ext_color(ap[i])];
		}
...
errr init_ibm(int argc, char **argv)
{
	...
	t->pict_hook = Term_pict_ibm;
        text_iswprint_hook = Term_iswprint_ibm;
        text_wcschr_hook = Term_wcschr_ibm;
        text_wcslen_hook = Term_wcslen_ibm;
